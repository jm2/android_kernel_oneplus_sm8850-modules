...
| `load-fail-crc` | 0 | 0 |
| `load-fail-module-layout` | 576 | 313 |
| `no-versions` | 0 | 0 |
| `excluded-source-built-override` | 0 | 0 |
| **TOTAL** | **652** | **389** |

**Phase 6 boot prediction (today):** 76/389 modules in modules.load resolve cleanly (19.5%).
...
**Phase 6 boot prediction (after `module_layout` fix):** 76/389 = 19.5%. The delta between 19.5% (today) and 19.5% (post-fix) is the leverage of the single-symbol fix.
...
**`module_layout`: 313 boot modules** (the veto symbol — single source of the catastrophic failure; see veto section above). Listed separately so subsequent rows show the second-most-leveraged CRC gaps.
...
| flipped: load-clean → load-fail (REGRESSION) | 19 |
| changed verdict within load-fail-* | 0 |
| unchanged load-clean | 76 |
...
### REGRESSIONS — modules that were clean and now aren't

| Module | Subsystem | Was → Is |
|---|---|---|
| `clk-gate_test` | misc | `load-clean` → `load-fail-module-layout` |
| `clk-test` | misc | `load-clean` → `load-fail-module-layout` |
| `clk_kunit_helpers` | misc | `load-clean` → `load-fail-module-layout` |
| `dev_addr_lists_test` | misc | `load-clean` → `load-fail-module-layout` |
| `ext4-inode-test` | misc | `load-clean` → `load-fail-module-layout` |
| `fat_test` | misc | `load-clean` → `load-fail-module-layout` |
| `hid-uclogic-test` | misc | `load-clean` → `load-fail-module-layout` |
| `iio-test-format` | misc | `load-clean` → `load-fail-module-layout` |
| `input_test` | misc | `load-clean` → `load-fail-module-layout` |
| `kunit` | misc | `load-clean` → `load-fail-module-layout` |
| `kunit-example-test` | misc | `load-clean` → `load-fail-module-layout` |
| `kunit-test` | misc | `load-clean` → `load-fail-module-layout` |
| `lib_test` | misc | `load-clean` → `load-fail-module-layout` |
| `regmap-kunit` | misc | `load-clean` → `load-fail-module-layout` |
| `regmap-ram` | misc | `load-clean` → `load-fail-module-layout` |
| `regmap-raw-ram` | misc | `load-clean` → `load-fail-module-layout` |
| `soc-topology-test` | misc | `load-clean` → `load-fail-module-layout` |
| `soc-utils-test` | misc | `load-clean` → `load-fail-module-layout` |
| `time_test` | misc | `load-clean` → `load-fail-module-layout` |
