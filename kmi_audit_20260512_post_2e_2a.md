...
| `load-fail-crc` | 0 | 0 |
| `load-fail-module-layout` | 299 | 123 |
| `no-versions` | 0 | 0 |
| `excluded-source-built-override` | 258 | 189 |
| **TOTAL** | **652** | **389** |

**Phase 6 boot prediction (today):** 266/389 modules in modules.load resolve cleanly (68.4%).
...
| `load-fail-crc` | 123 |
| `no-versions` | 0 |
| `excluded-source-built-override` | 189 |

**Phase 6 boot prediction (after `module_layout` fix):** 266/389 = 68.4%. The delta between 68.4% (today) and 68.4% (post-fix) is the leverage of the single-symbol fix.
...
| `qcom_dcvs_register_voter` | 3 |
| `qcom_dcvs_hw_minmax_get` | 3 |
| `qcom_dcvs_update_votes` | 3 |
| `get_boot_mode` | 3 |
...
**`module_layout`: 123 boot modules** (the veto symbol — single source of the catastrophic failure; see veto section above). Listed separately so subsequent rows show the second-most-leveraged CRC gaps.

### Top 20 non-`module_layout` CRC mismatches

| Symbol | Boot modules referencing |
|---|---:|
| `__stack_chk_fail` | 111 |
| `_printk` | 103 |
| `__platform_driver_register` | 82 |
| `platform_driver_unregister` | 79 |
| `devm_kmalloc` | 74 |
| `mem_alloc_profiling_key` | 67 |
| `mutex_lock` | 66 |
| `mutex_unlock` | 66 |
| `_dev_err` | 66 |
| `kfree` | 64 |
| `of_property_read_variable_u32_array` | 59 |
| `scnprintf` | 57 |
| `__mutex_init` | 56 |
| `kmalloc_caches` | 56 |
| `__kmalloc_cache_noprof` | 56 |
| `memcpy` | 51 |
| `memset` | 48 |
| `of_find_property` | 47 |
| `__list_add_valid_or_report` | 45 |
| `__fortify_panic` | 45 |
