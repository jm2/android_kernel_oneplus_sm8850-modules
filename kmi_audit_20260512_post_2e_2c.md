...
| `load-fail-crc` | 0 | 0 |
| `load-fail-module-layout` | 296 | 121 |
| `no-versions` | 0 | 0 |
| `excluded-source-built-override` | 261 | 191 |
| **TOTAL** | **652** | **389** |

**Phase 6 boot prediction (today):** 268/389 modules in modules.load resolve cleanly (68.9%).
...
| `load-fail-crc` | 121 |
| `no-versions` | 0 |
| `excluded-source-built-override` | 191 |

**Phase 6 boot prediction (after `module_layout` fix):** 268/389 = 68.9%. The delta between 68.9% (today) and 68.9% (post-fix) is the leverage of the single-symbol fix.
...
**`module_layout`: 121 boot modules** (the veto symbol — single source of the catastrophic failure; see veto section above). Listed separately so subsequent rows show the second-most-leveraged CRC gaps.

### Top 20 non-`module_layout` CRC mismatches

| Symbol | Boot modules referencing |
|---|---:|
| `__stack_chk_fail` | 109 |
| `_printk` | 101 |
| `__platform_driver_register` | 80 |
| `platform_driver_unregister` | 77 |
| `devm_kmalloc` | 72 |
| `mem_alloc_profiling_key` | 66 |
| `mutex_lock` | 65 |
| `mutex_unlock` | 65 |
| `_dev_err` | 64 |
| `kfree` | 63 |
| `of_property_read_variable_u32_array` | 57 |
| `scnprintf` | 56 |
| `__mutex_init` | 55 |
| `kmalloc_caches` | 55 |
| `__kmalloc_cache_noprof` | 55 |
| `memcpy` | 50 |
| `memset` | 47 |
| `of_find_property` | 45 |
| `__list_add_valid_or_report` | 44 |
| `__fortify_panic` | 44 |
