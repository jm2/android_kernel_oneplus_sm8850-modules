...
| `load-fail-crc` | 0 | 0 |
| `load-fail-module-layout` | 294 | 119 |
| `no-versions` | 0 | 0 |
| `excluded-source-built-override` | 263 | 193 |
| **TOTAL** | **652** | **389** |

**Phase 6 boot prediction (today):** 270/389 modules in modules.load resolve cleanly (69.4%).
...
| `load-fail-crc` | 119 |
| `no-versions` | 0 |
| `excluded-source-built-override` | 193 |

**Phase 6 boot prediction (after `module_layout` fix):** 270/389 = 69.4%. The delta between 69.4% (today) and 69.4% (post-fix) is the leverage of the single-symbol fix.
...
**`module_layout`: 119 boot modules** (the veto symbol — single source of the catastrophic failure; see veto section above). Listed separately so subsequent rows show the second-most-leveraged CRC gaps.

### Top 20 non-`module_layout` CRC mismatches

| Symbol | Boot modules referencing |
|---|---:|
| `__stack_chk_fail` | 107 |
| `_printk` | 99 |
| `__platform_driver_register` | 78 |
| `platform_driver_unregister` | 75 |
| `devm_kmalloc` | 70 |
| `_dev_err` | 64 |
| `mem_alloc_profiling_key` | 64 |
| `mutex_lock` | 63 |
| `mutex_unlock` | 63 |
| `kfree` | 62 |
| `of_property_read_variable_u32_array` | 55 |
| `kmalloc_caches` | 54 |
