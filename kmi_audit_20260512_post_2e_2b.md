...
| `load-fail-crc` | 0 | 0 |
| `load-fail-module-layout` | 297 | 122 |
| `no-versions` | 0 | 0 |
| `excluded-source-built-override` | 260 | 190 |
| **TOTAL** | **652** | **389** |

**Phase 6 boot prediction (today):** 267/389 modules in modules.load resolve cleanly (68.6%).
...
| `load-fail-crc` | 122 |
| `no-versions` | 0 |
| `excluded-source-built-override` | 190 |

**Phase 6 boot prediction (after `module_layout` fix):** 267/389 = 68.6%. The delta between 68.6% (today) and 68.6% (post-fix) is the leverage of the single-symbol fix.
...
**`module_layout`: 122 boot modules** (the veto symbol — single source of the catastrophic failure; see veto section above). Listed separately so subsequent rows show the second-most-leveraged CRC gaps.

### Top 20 non-`module_layout` CRC mismatches

| Symbol | Boot modules referencing |
|---|---:|
| `__stack_chk_fail` | 110 |
| `_printk` | 102 |
| `__platform_driver_register` | 81 |
| `platform_driver_unregister` | 78 |
| `devm_kmalloc` | 73 |
...
| `_dev_err` | 65 |
| `kfree` | 64 |
| `of_property_read_variable_u32_array` | 58 |
| `scnprintf` | 57 |
| `__mutex_init` | 56 |
| `kmalloc_caches` | 56 |
