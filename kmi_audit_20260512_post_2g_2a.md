...
| `load-fail-crc` | 0 | 0 |
| `load-fail-module-layout` | 295 | 120 |
| `no-versions` | 0 | 0 |
| `excluded-source-built-override` | 262 | 192 |
| **TOTAL** | **652** | **389** |

**Phase 6 boot prediction (today):** 269/389 modules in modules.load resolve cleanly (69.2%).
...
| `load-fail-crc` | 120 |
| `no-versions` | 0 |
| `excluded-source-built-override` | 192 |

**Phase 6 boot prediction (after `module_layout` fix):** 269/389 = 69.2%. The delta between 69.2% (today) and 69.2% (post-fix) is the leverage of the single-symbol fix.
...
**`module_layout`: 120 boot modules** (the veto symbol — single source of the catastrophic failure; see veto section above). Listed separately so subsequent rows show the second-most-leveraged CRC gaps.

### Top 20 non-`module_layout` CRC mismatches

| Symbol | Boot modules referencing |
|---|---:|
| `__stack_chk_fail` | 108 |
| `_printk` | 100 |
| `__platform_driver_register` | 79 |
| `platform_driver_unregister` | 76 |
| `devm_kmalloc` | 71 |
| `mem_alloc_profiling_key` | 65 |
| `mutex_lock` | 64 |
| `mutex_unlock` | 64 |
| `_dev_err` | 64 |
| `kfree` | 62 |
| `of_property_read_variable_u32_array` | 56 |
| `scnprintf` | 55 |
| `__mutex_init` | 54 |
| `kmalloc_caches` | 54 |
| `__kmalloc_cache_noprof` | 54 |
| `memcpy` | 49 |
| `memset` | 46 |
| `of_find_property` | 44 |
| `__list_add_valid_or_report` | 43 |
| `__fortify_panic` | 43 |
